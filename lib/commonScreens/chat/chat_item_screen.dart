...
import 'package:gps_massageapp/customLibraryClasses/chat/reply_message_preview.dart';
import 'package:gps_massageapp/customLibraryClasses/chat/selected_media_preview.dart';
...
class _ChatItemScreenState extends State<ChatItemScreen>
  ...
  Future<dynamic> _updateTypingStatus(bool status, var peerId) {
    ...
    return FirebaseFirestore.instance.runTransaction((transaction) async {
      ...
      updateUserAppBar(status, peerId);
    });
  }

  updateUserAppBar(bool status, var peerId) {
    if (this.mounted) {
      setState(() {
        return MyAppBar(status, widget.chatData.peer, peerId);
      });
    }
  }
    ...
    return SafeArea(
      ...
      child: Scaffold(
        ...
        appBar: PreferredSize(
          ...
          child: MyAppBar(
              _isTyping, widget.chatData.peer, widget.chatData.groupId),
        ),
...
class ChatOps {
  ...
  // if last message in a series from same UserDetail
  ...
  // if message is in the series of messages of one UserDetail
  ...
  void addNewMessages(AsyncSnapshot<dynamic> snapshots) {
    if (snapshots.hasData) {
      ...
      for (int i = 0; i < snapshots.data.documents.length; i++) {
        ...
        Future.doWhile(() {
          Message newMsg =
              Message.fromMap(Map<String, dynamic>.from(snapshot.data())
                  /* snapshot.data */);
          if (widget.chatData.messages.isNotEmpty) {
            // add message to the list only if it's after the first item in the list
            if (newMsg.sendDate.isAfter(widget.chatData.messages[0].sendDate)) {
